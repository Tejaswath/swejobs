...
import { BookOpen, ChevronDown, GraduationCap, TrendingUp } from "lucide-react";
...
const STUDY_FOCUS_COLLAPSE_KEY = "swejobs.study-focus.expanded";
...
export function StudyFocusPanel({
  ...
  useEffect(() => {
    const stored = localStorage.getItem(STUDY_FOCUS_COLLAPSE_KEY);
    setIsExpanded(stored === "true");
  }, []);

  useEffect(() => {
    localStorage.setItem(STUDY_FOCUS_COLLAPSE_KEY, String(isExpanded));
  }, [isExpanded]);
        ...
        {unavailable ? (
          ...
          <div className="mt-5 rounded-[24px] border border-dashed border-border/60 bg-background/35 p-5">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Skill signals will appear once the first market digest is generated. Add your skills in Skill Gap to compare as soon as data is available.
            </p>
            <Link to="/skills" className="mt-3 inline-flex items-center gap-1.5 text-sm text-primary hover:text-primary/80">
              Open Skill Gap
              <TrendingUp className="h-3.5 w-3.5" />
            </Link>
          </div>
        ) : (
          <Collapsible open={isExpanded} onOpenChange={setIsExpanded} className="mt-5 rounded-[24px] border border-border/50 bg-background/35 p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="min-w-0 space-y-1">
                <p className="font-mono text-[11px] uppercase tracking-[0.22em] text-muted-foreground">Primary focus</p>
                <p className="truncate text-lg font-semibold text-foreground">{primarySkill}</p>
                <p className="truncate text-sm text-muted-foreground">
                  Pair with {secondarySkill}
                </p>
              </div>
